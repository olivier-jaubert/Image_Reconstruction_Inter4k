...
class LocalConvBlock(tf.keras.layers.Layer):
  ...
  Args:
    ...
    time_distributed: A `boolean`. If `True`, wraps different layers with 
                      time_distributed layers for 2D RNN. Defaults to False
  ...
  def __init__(self,
    ...
    if self._time_distributed:
      conv= lambda *args, **kwargs: tf.keras.layers.TimeDistributed(layer_util.get_nd_layer('Conv', self._rank)(*args, **kwargs))
    ...
    if self._time_distributed:
      if self._use_dropout:
        dropout= lambda *args, **kwargs: tf.keras.layers.TimeDistributed(dropout(*args, **kwargs))
      if self._use_batch_norm:
        bn= lambda *args, **kwargs: tf.keras.layers.TimeDistributed(bn(*args, **kwargs))
...
class PixelShuffle(tf.keras.layers.Layer):
  def __init__(self, 
    ...
    self._conv=conv(filters=self._filters*self._pool_size**(self._rank-1),
               kernel_size=self._kernel_size,
               strides=self._strides,
               padding='same',
               data_format=None,
  ...
  def call(self, inputs, training=None): # pylint: disable=unused-argument, missing-param-doc
    ...
    x = inputs
    x = self._conv(x)
    x = tf.nn.depth_to_space(x, self._pool_size)
